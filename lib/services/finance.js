...
export async function getFinancialData(ticker) {
  try {
    ...
    const quote = await yahooFinance.quote(cleanTicker).catch((err) => {
      console.error(`[GET /api/finance] Quote failed for ${cleanTicker}:`, err);
      return null;
    });

    if (!quote) {
      console.warn(`[GET /api/finance] Quote returned null for ${cleanTicker}. Falling back to mock data.`);
      return getMockFinancialData(cleanTicker);
    }
    ...
    const rawHistorical = await yahooFinance.historical(cleanTicker, {
      ...
      console.error(`[GET /api/finance] Historical failed for ${cleanTicker}:`, err);
      return [];
    });
    ...
    const summary = await yahooFinance.quoteSummary(cleanTicker, {
      ...
      console.error(`[GET /api/finance] QuoteSummary failed for ${cleanTicker}:`, err);
      return null;
    });
    ...
    const financialMetrics = {
      ...
      regularMarketChangePercent: quote?.regularMarketChangePercent ?? 0.0,
      displayName: quote?.displayName || quote?.longName || cleanTicker,
    };
...
function getMockFinancialData(ticker) {
  ...
  return {
    ...
    financialMetrics: {
      currentPrice,
      marketCap: symbol === 'AAPL' ? 2950.4 : symbol === 'TSLA' ? 560.2 : symbol === 'NVDA' ? 2190.8 : 125.4,
      peRatio,
      eps: symbol === 'AAPL' ? 6.13 : symbol === 'TSLA' ? 4.31 : symbol === 'NVDA' ? 12.44 : 7.38,
      profitMargin: symbol === 'AAPL' ? 25.8 : symbol === 'TSLA' ? 14.5 : symbol === 'NVDA' ? 48.2 : 16.5,
      debtToEquity: symbol === 'AAPL' ? 1.45 : symbol === 'TSLA' ? 0.08 : symbol === 'NVDA' ? 0.17 : 0.72,
      fiftyTwoWeekRange: symbol === 'AAPL' ? '164.08 - 199.62' : symbol === 'TSLA' ? '138.80 - 299.29' : symbol === 'NVDA' ? '373.56 - 974.00' : '110.50 - 180.25',
